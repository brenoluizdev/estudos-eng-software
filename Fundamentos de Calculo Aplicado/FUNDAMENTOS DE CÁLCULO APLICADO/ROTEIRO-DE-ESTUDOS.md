# Roteiro de Estudos — Fundamentos de Cálculo Aplicado

Gerado a partir da leitura de todos os `content.md` das 4 unidades (20 aulas). Duas listas:

1. **Lista 1** — todos os tópicos/conceitos matemáticos cobertos no curso (visão geral, sem checkbox).
2. **Lista 2** — checklist de estudo por aula, com vídeos recomendados de bons professores no YouTube. Marque `[x]` quando assistir a aula e quando dominar cada ponto.

> Os canais indicados abaixo foram verificados via busca (nome, foco de conteúdo e reputação). Quando não há um vídeo específico linkado, pesquise o termo indicado dentro do canal recomendado — a maioria organiza o conteúdo em playlists por assunto.

---

## Lista 1 — Todos os tópicos da disciplina

### Unidade 1 — Fundamentos Gerais de Matemática
- Teoria dos conjuntos: definição, notação, elementos
- Conjunto dos naturais (ℕ)
- Conjunto dos inteiros (ℤ) e subconjuntos (ℤ₊, ℤ₋, ℤ*, etc.)
- Conjunto dos racionais (ℚ) e frações equivalentes
- Conjunto dos irracionais (𝕀) — π e número de Euler (e)
- Conjunto dos reais (ℝ = ℚ ∪ 𝕀) e diagrama de Venn
- Operações com inteiros (adição, subtração, multiplicação, divisão euclidiana — quociente e resto)
- Operações com frações (adição, subtração, multiplicação, divisão, MMC)
- Números complexos (ℂ): forma a+bi, unidade imaginária i²=−1
- Plano de Argand-Gauss (representação geométrica de complexos)
- Equação: incógnita, membros, equações equivalentes
- Equações de 1º grau (lineares)
- Equações de 2º grau (quadráticas) — **fórmula de Bhaskara**
- Discriminante (Δ = b²−4ac) e análise do tipo de solução
- Inequações de 1º grau e suas propriedades operatórias
- Representação gráfica de solução de inequação (reta numérica)
- Inequações com duas variáveis (representação no plano)
- Potências com expoente inteiro (positivo e negativo)
- Propriedades das potências (produto, quociente, potência de potência, de produto/quociente)
- Potências de expoente racional (relação com radicais)
- Definição de logaritmo (log_a b = c ⟺ a^c = b)
- Propriedades operatórias de logaritmos (produto, quociente, potência)
- Logaritmo decimal (base 10) e logaritmo natural/neperiano (base e, notação ln)
- Elementos do triângulo, soma dos ângulos internos = 180°
- Triângulo retângulo: hipotenusa e catetos
- Teorema de Pitágoras
- Graus e radianos (conversão)
- Razões trigonométricas: seno, cosseno, tangente
- Ângulos notáveis (30°, 45°, 60°)
- Razões trigonométricas inversas: secante, cossecante, cotangente

### Unidade 2 — Fundamentos Gerais sobre Funções
- Conceito de função: domínio, contradomínio, imagem, lei de formação
- Representações de função (diagrama de flechas, gráfico, tabela, algébrica)
- Função polinomial (coeficientes, grau)
- Função afim (1º grau): coeficiente angular e linear
- Casos particulares: função linear e função constante
- Crescimento/decrescimento e raiz da função afim
- Função quadrática (2º grau) e concavidade da parábola
- Discriminante e tipos de raízes da função quadrática
- Vértice da parábola (xv, yv) — valor máximo/mínimo
- Função exponencial f(x) = aˣ e seu gráfico
- Equações exponenciais (igualdade de potências de mesma base)
- Função exponencial de base e
- Função logarítmica f(x) = log_a x e seu gráfico
- Mudança de base de logaritmo
- Relação entre função exponencial e logarítmica (funções inversas, simetria em y=x)
- Equações logarítmicas (estratégias de resolução)
- Uso de logaritmo para resolver equações exponenciais
- Ciclo/circunferência trigonométrica e quadrantes
- Ângulos notáveis no ciclo trigonométrico (em radianos)
- Função seno e função cosseno (periodicidade, imagem [−1,1])
- Parâmetros de transformação de senoides (amplitude, período, deslocamento vertical/horizontal)
- Função tangente (domínio restrito, período π)

### Unidade 3 — Fundamentos Gerais sobre Limites e Derivadas
- Noção intuitiva e definição formal de limite (ε–δ)
- Limite bilateral e limites laterais (esquerda/direita)
- Unicidade do limite
- Propriedades operatórias dos limites (soma, produto, quociente, potência, raiz)
- Limite de função polinomial (substituição direta)
- Teorema do confronto (sanduíche)
- Limites infinitos e assíntota vertical
- Limites no infinito e assíntota horizontal
- Limites de funções racionais no infinito (comparação de graus)
- Limites de exponenciais e logaritmos no infinito
- Limite fundamental trigonométrico: lim sen(x)/x = 1 (x→0)
- Limite exponencial fundamental: lim (1+1/x)ˣ = e
- Indeterminações (∞−∞, ∞/∞, 0/0, 0·∞)
- Definição de função contínua em um ponto (3 condições)
- Continuidade em intervalo e continuidade lateral
- Descontinuidade removível, de salto e infinita
- Teorema do Valor Intermediário (TVI)
- Problema da reta tangente (retas secantes → tangente)
- Taxa de variação e velocidade instantânea
- Definição de derivada em um ponto (quociente de Newton / limite)
- Derivada como função e notações (f', Dx, dy/dx)
- Derivadas de ordem superior (2ª ordem, etc.)
- Regras básicas de derivação: constante, regra para x, regra da potência, linearidade
- Relação entre derivabilidade e continuidade
- Derivadas laterais

### Unidade 4 — Fundamentos Gerais sobre Cálculo Diferencial e Integral
- Regra do produto
- Regra do quociente
- Regra da potência com expoente racional
- **Regra da cadeia** (função composta)
- Derivada da função exponencial (eˣ e bˣ)
- Derivada da função logarítmica (ln x e log_b x)
- Derivada de função inversa
- Derivadas das funções trigonométricas (sen, cos, tg, cotg, sec, cossec)
- Máximos e mínimos locais e globais (absolutos)
- Teorema do Valor Extremo
- Ponto crítico
- Teste de crescimento/decrescimento (sinal da 1ª derivada)
- Concavidade e teste da concavidade
- Teste da primeira derivada
- Teste da segunda derivada
- Ponto de inflexão
- Soma de Riemann (soma superior e inferior)
- Integral definida e suas propriedades
- Teorema Fundamental do Cálculo (Parte 1 e Parte 2)
- Antiderivada / primitiva
- Integral indefinida (constante de integração C)
- Tabela de integrais imediatas
- **Integração por substituição**
- **Integração por partes**
- Área entre curvas (via integral definida)
- Aplicações de integrais: volume de sólido de revolução, trabalho de uma força
- Taxas relacionadas (regra da cadeia aplicada a variáveis em função do tempo)

---

## Lista 2 — Checklist de estudo com vídeos recomendados

Cada tópico tem o link direto do melhor vídeo encontrado na busca (título, canal e foco do conteúdo conferidos antes de incluir).

> Canais preferidos (nessa ordem de prioridade, quando houver vídeo adequado ao tópico): 1. [@sandrocuriodicasdemat](https://www.youtube.com/@sandrocuriodicasdemat) (Dicasdemat Sandro Curió), 2. [@equacionamatematica](https://www.youtube.com/@equacionamatematica) (Equaciona Com Paulo Pereira), 3. @explicabem. Nas aulas ainda não assistidas (Unidades 2-4), os vídeos abaixo foram atualizados priorizando esses canais. Nas aulas já assistidas (Unidade 1), os vídeos originais foram mantidos e — onde havia vídeo desses canais cobrindo o mesmo tópico — foi adicionada uma referência extra com checkbox próprio.
>
> ⚠️ **Verificado:** o canal do Sandro Curió (@sandrocuriodicasdemat) **não tem conteúdo de Cálculo Diferencial e Integral** (checado via playlists, cursos e busca interna por "derivada" em 2026-08-25) — é focado em matemática do ensino médio/ENEM/concursos, sem nenhuma playlist/curso sobre limites, derivadas ou integrais. Por isso, da Aula 4 da Unidade 3 em diante (limites → derivadas → Unidade 4 inteira), os vídeos seguem apenas com Equaciona Com Paulo Pereira (@equacionamatematica) e outros canais especializados em Cálculo 1 como 2ª opção.

### Unidade 1 — Fundamentos Gerais de Matemática

- [x] **Aula 1 assistida** — Conjuntos Numéricos e Operações
  - [x] Conjuntos numéricos (ℕ, ℤ, ℚ, 𝕀, ℝ, ℂ): https://www.youtube.com/watch?v=yZP9rLJQRqA
  - [x] Operações com frações e MMC: https://www.youtube.com/watch?v=HunWcKjwRzs
  - [x] Números complexos e plano de Argand-Gauss (Me Salva!): https://www.youtube.com/watch?v=RDA71S_2CC8

- [x] **Aula 2 assistida** — Equações e Inequações
  - [x] Equações de 1º grau: https://www.youtube.com/watch?v=HIWC8RU8Ze0
  - [x] Equações de 2º grau / fórmula de Bhaskara (Lucas Borguezan): https://www.youtube.com/watch?v=9_7M5k2tWiA
  - [x] Equações de 2º grau / Bhaskara — Dicasdemat Sandro Curió (@sandrocuriodicasdemat): https://www.youtube.com/watch?v=ZZ9gGsqwrpo
  - [x] Inequações de 1º grau (uma e duas variáveis, solução gráfica): https://www.youtube.com/watch?v=0n-o8B1hoN4
  - [x] Inequações de 1º grau — Dicasdemat Sandro Curió (@sandrocuriodicasdemat): https://www.youtube.com/watch?v=qrnOtSD1vF8

- [x] **Aula 3 assistida** — Potências e Logaritmos
  - [x] Propriedades de potências: https://www.youtube.com/watch?v=pzZXpIziFzc
  - [x] Propriedades de potências ("Potenciação em 10 minutos") — Dicasdemat Sandro Curió (@sandrocuriodicasdemat), ~10min: https://www.youtube.com/watch?v=FzkAWvOAEUI
  - [x] Definição e propriedades de logaritmos: https://www.youtube.com/watch?v=qMv1AgBnsUE
  - [x] Logaritmo natural (ln): https://www.youtube.com/watch?v=2YVoS2E-_7c

- [x] **Aula 4 assistida** — Tópicos de Trigonometria
  - [x] Teorema de Pitágoras (demonstração geométrica): https://www.youtube.com/watch?v=2NOzQtdoSZg
  - [x] Graus e radianos (aula completa + exercícios): https://www.youtube.com/watch?v=4uVr4sWMMtU
  - [x] Círculo trigonométrico (contexto de graus/radianos) — Dicasdemat Sandro Curió (@sandrocuriodicasdemat): https://www.youtube.com/watch?v=9FgfEQM1a3c
  - [x] Razões trigonométricas (seno, cosseno, tangente) e ângulos notáveis: https://www.youtube.com/watch?v=4sTUs4ll3dI
  - [x] * Razões trigonométricas no triângulo retângulo ("em 13 minutos") — Dicasdemat Sandro Curió (@sandrocuriodicasdemat), ~13min: https://www.youtube.com/watch?v=C7NrVLmEYcs

- [x] **Aula 5 assistida** — Encerramento da Unidade (revisão dos pontos acima)

### Unidade 2 — Fundamentos Gerais sobre Funções

- [x] **Aula 1 assistida** — Funções Afim e Quadrática
  - [x] Conceito de função (domínio, contradomínio, imagem) — Dicasdemat Sandro Curió: https://www.youtube.com/watch?v=GkuG8xZrYbs
  - [x] Função afim (gráfico, coef. angular/linear, raiz) — Dicasdemat Sandro Curió: https://www.youtube.com/watch?v=JgGseIonmYI
  - [x] Função quadrática (gráfico, parábola, vértice) — Dicasdemat Sandro Curió, ~21min: https://www.youtube.com/watch?v=ZpW9Xb5iyt4

- [x] **Aula 2 assistida** — Função Exponencial
  - [x] Função exponencial: definição e gráfico — Dicasdemat Sandro Curió, ~15min: https://www.youtube.com/watch?v=zU2nPaGyauc

- [x] **Aula 3 assistida** — Função Logarítmica
  - [x] Função logarítmica: definição, gráfico e propriedades — Dicasdemat Sandro Curió: https://www.youtube.com/watch?v=oza6zrCMOPM
  - [x] Logaritmo — mudança de base (base para equações logarítmicas) — Dicasdemat Sandro Curió: https://www.youtube.com/watch?v=MYOLeCwp_kc
  - [x] Relação função exponencial ↔ logarítmica (funções inversas): https://www.youtube.com/watch?v=Gg-xMSMI4VY

- [x] **Aula 4 assistida** — Função Trigonométrica
  - [x] Ciclo trigonométrico — Dicasdemat Sandro Curió: https://www.youtube.com/watch?v=9FgfEQM1a3c
  - [x] Função seno (gráfico, imagem, período) — Dicasdemat Sandro Curió: https://www.youtube.com/watch?v=FwTbL4hewlY
  - [x] Função cosseno e transformações (amplitude, período, deslocamento) — Dicasdemat Sandro Curió: https://www.youtube.com/watch?v=vUvWnRDcBrc

- [x] **Aula 5 assistida** — Encerramento da Unidade (revisão dos pontos acima — reaproveitar os vídeos das Aulas 1-4)

### Unidade 3 — Fundamentos Gerais sobre Limites e Derivadas

> Caso você tenha dificuldade em entender limites assim como eu tive ao assistir as aulas da Prof. da faculdade, ou pelas aulas no youtube, recomendo esse site: 
> ### https://pt.khanacademy.org/math/ap-calculus-ab
> ~

- [x] **Aula 1 assistida** — Introdução aos Limites
  - [x] Definição de limite (Cálculo 1 #3) — Equaciona Com Paulo Pereira (@equacionamatematica), ~8min: https://www.youtube.com/watch?v=voBexx2V7gw
  - [x] Limites laterais (Cálculo 1 #7) — Equaciona Com Paulo Pereira (@equacionamatematica), ~8min: https://www.youtube.com/watch?v=HwHO_w6V_No
  - [x] Propriedades dos limites e teorema do confronto (sanduíche) — Grings, ~15min: https://www.youtube.com/watch?v=k3FOy1uibO8

- [ ] **Aula 2 assistida** — Limites Infinitos e no Infinito
  - [x] Limites no infinito e limites infinitos (Cálculo 1 #9) — Equaciona Com Paulo Pereira (@equacionamatematica), ~10min: https://www.youtube.com/watch?v=8uWVkjgzB7A
  - [x] Limites no infinito de função racional, com macete (Cálculo 1 #10) — Equaciona Com Paulo Pereira (@equacionamatematica), ~8min: https://www.youtube.com/watch?v=nX6XtwX5dV4
  - [ ] Limite fundamental trigonométrico sen(x)/x (Cálculo 1 #12) — Equaciona Com Paulo Pereira (@equacionamatematica), ~12min: https://www.youtube.com/watch?v=nxpW6EqN2Jc

- [x] **Aula 3 assistida** — Continuidade de Funções
  - [x] Funções contínuas — definição e tipos de descontinuidade (Cálculo 1 #13) — Equaciona Com Paulo Pereira (@equacionamatematica), ~8min: https://www.youtube.com/watch?v=PW_Y2pvJg4s
  - [x] Propriedades das funções contínuas / continuidade em intervalo (Cálculo 1 #14) — Equaciona Com Paulo Pereira (@equacionamatematica), ~8min: https://www.youtube.com/watch?v=4L4E_N3EaJs
  - [x] Teorema do Valor Intermediário (Cálculo 1 #15) — Equaciona Com Paulo Pereira (@equacionamatematica), ~8min: https://www.youtube.com/watch?v=xtAd5bI5LyQ

- [ ] **Aula 4 assistida** — Introdução às Derivadas
  - [x] Derivada — definição e cálculo, reta tangente (Cálculo 1 #16) — Equaciona Com Paulo Pereira (@equacionamatematica), ~10min: https://www.youtube.com/watch?v=cWBEMN75IMc
  - [ ] Regras básicas de derivação (Cálculo 1 #18) — Equaciona Com Paulo Pereira (@equacionamatematica), ~10min: https://www.youtube.com/watch?v=2NBVVW3XvZc
  - [ ] Relação entre derivabilidade e continuidade — Grings, ~12min: https://www.youtube.com/watch?v=6qUo0cdnvFg

- [ ] **Aula 5 assistida** — Encerramento da Unidade (revisão dos pontos acima — reaproveitar os vídeos das Aulas 1-4)

### Unidade 4 — Fundamentos Gerais sobre Cálculo Diferencial e Integral

- [x] **Aula 1 assistida** — Regras de Derivação
  - [x] Regra do produto — Prof. Murakami, ~4min: https://www.youtube.com/watch?v=CkWcC5PxAIs
  - [x] Regra do quociente — Prof. Murakami, ~14min: https://www.youtube.com/watch?v=QwYxbUHhsuI
  - [x] Regra da cadeia (Cálculo 1 #19) — Equaciona Com Paulo Pereira (@equacionamatematica): https://www.youtube.com/watch?v=b5m864fIKvA
  - [x] Derivada da exponencial e do logaritmo (Cálculo 1 #21) — Equaciona Com Paulo Pereira (@equacionamatematica): https://www.youtube.com/watch?v=91UN2cbzBGY (derivadas trigonométricas específicas não encontradas nos canais prioritários — pesquisar à parte)

- [ ] **Aula 2 assistida** — Derivadas e Otimização
  - [ ] Pontos críticos, extremos e teste da 1ª derivada — máximos e mínimos (Cálculo 1 #34) — Equaciona Com Paulo Pereira (@equacionamatematica): https://www.youtube.com/watch?v=dDwWaoJlSlA
  - [ ] Concavidade e pontos de inflexão — teste da 2ª derivada (Cálculo 1 #37) — Equaciona Com Paulo Pereira (@equacionamatematica): https://www.youtube.com/watch?v=PMOEMs00Jz4

- [ ] **Aula 3 assistida** — Introdução às Integrais
  - [x] Integral definida: áreas e soma de Riemann (Cálculo 1 #44) — Equaciona Com Paulo Pereira (@equacionamatematica): https://www.youtube.com/watch?v=CWWbjoOjYOg
  - [ ] Teorema Fundamental do Cálculo (Cálculo 1 #45) — Equaciona Com Paulo Pereira (@equacionamatematica): https://www.youtube.com/watch?v=qRMvrwLt96s
  - [ ] Integral indefinida / antiderivada (Cálculo 1 #41) — Equaciona Com Paulo Pereira (@equacionamatematica): https://www.youtube.com/watch?v=M_xCxHcBdBo

- [ ] **Aula 4 assistida** — Calculando Integrais
  - [ ] Integração por substituição — método infalível (Cálculo 1 #42) — Equaciona Com Paulo Pereira (@equacionamatematica): https://www.youtube.com/watch?v=fHom2rFJGjg
  - [ ] Integração por partes (Cálculo 1 #43) — Equaciona Com Paulo Pereira (@equacionamatematica): https://www.youtube.com/watch?v=E3ZILV7ER54
  - [ ] Área entre curvas — BOTE FÉ NA MATEMÁTICA (nenhum dos 3 canais prioritários tinha conteúdo específico): https://www.youtube.com/watch?v=hBRCUKM-9p4

- [ ] **Aula 5 assistida** — Encerramento da Unidade
  - [ ] Taxas relacionadas — Isaías Lima Matemática (nenhum dos 3 canais prioritários tinha conteúdo específico): https://www.youtube.com/watch?v=1FUq77TB18U

---

### Como usar este roteiro
1. Assista cada aula (marque o checkbox "Aula assistida").
2. Para cada tópico, veja o vídeo já linkado ou pesquise o termo sugerido dentro do canal indicado.
3. Depois de resolver os exercícios da seção "Vamos Exercitar?" da aula correspondente sem consultar a resolução, marque o tópico como concluído.
